...
> **Lot W4-C — opératoire.** Ce document ne livre aucune ligne de code et ne
...
| E4 | Relecture stable entre 1 s et 10 s après l'écriture | I-7 bornée empiriquement |
...
| I-6 | durée réelle d'une écriture | **oui**, mesurée |

Ce qu'elle **ne lèvera pas**, et il faut le dire avant d'exécuter :

| Réf | Inconnue | Pourquoi elle résiste |
|---|---|---|
| I-7 | délai avant relecture fiable | une écriture **à l'identique** ne propage aucun changement : il n'y a rien à attendre. Reste à la valeur empirique E4 |
...
4. **Arrêter le superviseur** — arrêter `<timer-guard>` (§8). *Vérifier* qu'il
   est bien inactif avant de continuer.
5. **Arrêter le pont** — arrêter `<unité-pont>`. Il sonde le démon toutes les
   ...
   suivi de redémarrage automatique : c'est bien un arrêt.
...
| 9 | preuve de retour arrière, armée avant l'essai | §12 |
| 10 | état du pont historique pendant l'essai | **arrêté**, ainsi que le superviseur (§8–§9) |

Le champ 8 est le seul livré en retrait de ce que W4-A espérait, et §6 dit
pourquoi : une écriture à l'identique ne propage rien. Ce n'est pas un défaut
d'exécution, c'est une limite de la méthode — et la méthode a été choisie parce
qu'elle est sûre.
